...
<div id="main" class="addProductToDbForm">
    ...
    <div id="productsForm">
        <?php
            ...
            echo $this->Form->input("image", array("type" => "file", "label" => __("image"), "accept" => "image/*"));
            echo $this->Form->input("subCategoryId",
				array(
					"options" => $subCategoriesIds,
					"label" => __("sub_category")
				)
			);
...
if ($this->Session->read("sizeError") == true) {
    echo "<script>Swal.fire({icon: 'error',text: '".__("file_limit_alert").
    ...
    echo "<script>Swal.fire({icon: 'error',text: '".__("price_error_alert").
		"',showConfirmButton: true,timer: 5000,timerProgressBar: true});</script>";
