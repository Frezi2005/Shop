<?php
    ...
    if ($this->Session->read("noticeRequestSent")) {
        echo "<script>Swal.fire({icon: 'success',text: '".__("notice_request_sent")."',showConfirmButton: true,timer: 5000,timerProgressBar: true});</script>";
        $_SESSION["noticeRequestSent"] = false;
    } else if ($this->Session->read("noticeExistsError")) {
        echo "<script>Swal.fire({icon: 'error',text: '".__("notice_request_exsits")."',showConfirmButton: true,timer: 5000,timerProgressBar: true});</script>";
