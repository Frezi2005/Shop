<?php
    ...
    echo "<input type='date' name='data[requestSickLeaveForm][start]' id='startDate' value='".date("Y-m-d").
		"' min='".date("Y-m-d")."'>";
    echo "</div>";
    echo "<div class='input date'>";
    echo "<label for='data[requestSickLeaveForm][end]'>".__("sick_leave_end_date")."</label>";
    echo "<input type='date' name='data[requestSickLeaveForm][end]' id='endDate' value='".date("Y-m-d")."' min='".
		date("Y-m-d")."'>";
    echo "</div>";
    echo $this->Form->end(__("submit"));

    if ($this->Session->read("sickLeaveRequestSent")) {
        echo "<script>Swal.fire({icon: 'success',text: '".__("sick_leave_request_sent").
        ...
        echo "<script>Swal.fire({icon: 'error',text: '".__("sick_leave_too_long").
			"',showConfirmButton: true,timer: 5000,timerProgressBar: true});</script>";
