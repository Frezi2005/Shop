...
<div id="main">
    ...
    <div id="changeEmailForm">
        <?php
            ...
            echo $this->Form->input("currentEmail",
				array(
					"type" => "email",
					"label" => "",
					"placeholder" => __("enter_current_email"),
					"required" => "true",
					"value" => $email
				)
			);
            echo $this->Form->input("newEmail",
				array(
					...
					"required" => "true",
					"value" => isset($_SESSION["data"]) && isset($_SESSION["data"]["newEmail"]) ?
						$_SESSION["data"]["newEmail"] :
						""
				)
			);
...
<?php
	if ($this->Session->read("changeEmailError")) {
		echo "<script>Swal.fire({icon: \"error\",text: '".__("change-email-error").
